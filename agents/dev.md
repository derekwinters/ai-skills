...
- **Resolved commands** (rendered per-repo from the distribution registry):
  - Test/red-green loop command: `{{ config.test_command? }}`
  - Validate/verify/build command: `{{ config.verify_command? }}`
...
- Run with `{{ config.test_command? }}` (or the repo's resolved test command).
...
### 5. Validate
- Run the repo's validator/build: `{{ config.verify_command? }}` (or the
...
- `{{ config.verify_command? }}` (or resolved validator) and the full suite pass ✓
